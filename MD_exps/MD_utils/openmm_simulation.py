...
def find_random_pdb(top_dir):
    pdb_file = f'{top_dir}/MD_exps/fs-pep/pdb/1FME-0.pdb'
    ...
    if(os.path.exists(dbfn)):
        try:
            with open(dbfn, 'rb') as f:
                db = pickle.load(f)
            pdb_file = db.next_random(m=120*2)
        except Exception as e:
            print("In find_random_pdb")
            print(e)

    return pdb_file, db


def md5_2_index(db, rmsd):
    if(db == None):
        return -10000
    for md5 in db.dictionary:
        if(rmsd == db.dictionary[md5][0]):
            break
    if(rmsd != db.dictionary[md5][0]):
        return -10000
    return db.sorted_index.index(md5)
...
def compute_RMSD(pdbfile, top_dir):
    outlier = mda.Universe(pdbfile)
...
def openmm_simulate_amber_fs_pep(pdb_file, top_file=None, check_point=None, GPU_index=0,
    ...
    while(not os.path.exists("../../../../aggregate/stop.aggregator")):
        ...
        if(not os.path.exists(stop_file)):
            ...
            td3 = os.path.abspath("../../../../")
            dbfn = f'{td3}/Outlier_search/OutlierDB.pickle'

            mylock1 = LockFile(dbfn)
            mylock1.acquire()

            pdb_file, db = find_random_pdb(td3)
            subprocess.getstatusoutput(f"cp {pdb_file} .")
            subprocess.getstatusoutput(f"cp {dbfn} .")
            """
            if(db != None):
                db.is_consistent()
            """
            #rmsd = compute_RMSD(pdb_file, td3)
            #rmsd_i = md5_2_index(db, rmsd)
            new_init = pmd.load_file(pdb_file).positions
            no_velocities = False
            try:
                velocities_init = np.load(pdb_file.replace('.pdb','.npy'))
                subprocess.getstatusoutput(f"cp {velocities_init} .")
            except:
                no_velocities = True
            mylock1.release()
            print(f"As initial condition using {pdb_file}")
            # print(f"with RMSD = {rmsd}, index = {rmsd_i}")
            # print(type(new_init)); sys.stdout.flush()
            # print(new_init)
            # print(dir(new_init))
            simulation.context.setPositions(new_init)
            if(no_velocities or random.random() > 0.5):
                print("Using random velocities")
                simulation.context.setVelocitiesToTemperature(300*u.kelvin, random.randint(1, 10000))
            else:
                print("Using saved velocities")
                simulation.context.setVelocities(velocities_init)
            v = simulation.context.getState(getVelocities=True).getVelocities(asNumpy=True)
            print(f"velocities: min = {v.min()}, max = {v.max()}, mean = {v.mean()}")
